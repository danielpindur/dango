...
public sealed class TofuGenerator : IIncrementalGenerator
{
        ...
        SourceProductionContext context)
    {
        ...
        foreach (var mappingsForEnumPair in mappingsForEnumPairs)
        {
            ...
            var mappingMethodName = GenerateExtensionMethodName(enumPair.SourceEnum, enumPair.DestinationEnum);
            
            sb.AppendLine($$"""
                public static {{enumPair.DestinationEnum.ToDisplayString()}} {{mappingMethodName}}(this {{enumPair.SourceEnum.ToDisplayString()}} value) =>
                    value switch
                    {
